...
function addMemeListeners() {
    ...
    elDownloadBtn.addEventListener('click', onDownloadImg)
...
function onDown(ev) {
    // Checking if selected line is clicked
    const { x, y } = getPos(ev)
...
function resizeCanvas() {
    ...
    gElCanvas.width = elCanvasContainer.offsetWidth - 6
...
function renderFocusedLineBorder() {
    ...
    const { x, y, size, txt, color } = focusedLine
    drawBorder(x, y, size, txt, color)
}

function drawBorder(x, y, size, txt, color) {
    ...
    // Keeping border in global variable in order to check if selected line is clicked
